...
import com.quantweb.mdserver.table.TableDataServer.MessageCase
...
abstract class TableDataServer(initialSchema: TableDataSchema) extends Actor {
  ...
  var tableData: Map[String, TableDataRow] = Map[String, TableDataRow]()
  ...
  def processMessage(msg: MessageCase): Unit = msg match {
    case UpdateTableDataRow(row) => {
      ...
      row getPrimaryKeyField (schema) match {
        //If the sent row has the primary key
        case Some(primaryKeyField) => {
          //Merge results
  ...
  override def receive = {
    case msg: MessageCase => processMessage(msg)
  }
}
...
object TableDataServer {
  ...
  case class ClientStartup(clientName: String) extends MessageCase

  //when client starts up, it sends this message to the server
  case class SendTableDataSchema(clientName: String) extends MessageCase

  //when client requests to send the schema
  case class SendEntireTableData(clientName: String) extends MessageCase

  //when client requests to send the entire data
  case class UpdateTableDataRow(row: TableDataRow) extends MessageCase

  //sent by a listener to forward the table data row to server's clients
